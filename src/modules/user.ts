...
export async function updateProfile(request: FastifyRequest<UpdateUserProps>, reply: FastifyReply) {
	try {
		const { authorization } = request.headers;
...
export async function updatePassword(request: FastifyRequest<UpdatePasswordProps>, reply: FastifyReply) {
	try {
		const { authorization } = request.headers;
...
export async function follow(request: FastifyRequest<FollowProps>, reply: FastifyReply) {
	try {
		const { authorization } = request.headers;
...
export async function unfollow(request: FastifyRequest<FollowProps>, reply: FastifyReply) {
	try {
		const { authorization } = request.headers;
		const { userId } = request.body;
		const me = await session(authorization);
